...
class CombineWords extends Component {
    constructor(props){
      super(props);
...
componentDidMount(){
  ...
  for(let qdata of questionDataResponse){
      if(qdata.id == this.state.questionList[0].id) {
          ...
          this.setState({ optionblock1 : qdata.selectedOptionId[0].value })
          this.setState({ optionblock2 : qdata.selectedOptionId[1].value })
          this.setState({ optionblock3 : qdata.selectedOptionId[2].value })
          this.setState({ optionblock4 : qdata.selectedOptionId[3].value })
          this.setState({ optionblock5 : qdata.selectedOptionId[4].value })
          this.setState({ optionblock6 : qdata.selectedOptionId[5].value })
      }
  }
  this.setState({questionDataResponse});
  // console.log("que res=== " ,this.state.questionDataResponse.slice(this.state.currentQuestion,this.state.currentQuestion+1));
...
getOptions = (item ,index, oid,qid,data) => {
  ...
  setTimeout(()=> {
    ...
    setTimeout(() => {
      ...
      this.props.update_answer_data(ans.toString())
...
export default connect(mapStateToProps,mapDispatchToProps)(CombineWords)