...
class MatchTheColumnsScreen extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            QuestionsColumn : [],
    ...
    componentDidMount(){
        ...
        // console.log("length of col A is === " , this.state.questionList[0].cola.length );

        setTimeout(()=>{
            // console.log(this.state.bQuestions);
            this.setState({
                data: this.state.bQuestions.map((d, index) => ({
                    key: `item-${d.value}`,
                    label: d,
        ...
        setTimeout(() => {
            ...
            for (let index = 0; index < this.state.colAQuestionNumber; index++) {
                // console.log("state data --- ",this.state.data[index]);
                anss.push(this.state.data[index].label.option_id)
                ansss.push(this.state.data[index].label.value)
            }
            setTimeout(() => {
                
                // console.log("anss data is=== ",anss);
                // console.log("ansss value is=== ",ansss);
            }, 2000);
            this.props.update_answer_data(anss.toString())
  ...
  render() {
    ...
    return (
      <>
      <div className="container col-12 col-sm-12 col-md-12 col-lg-10 mt-3 justify-content-center m-auto">
            ...
            >
              <div className="d-flex flex-column " >
                ...
                <div className="clob ">
                  ...
                  >
                    {this.state.bQuestions.map((data) => (
                        <div sm={12} md={3} className="">
                          <div
                            draggable
                            onDragEnd={({ data }) => console.log("dataa  --> ", data) }
...
export default connect(mapStateToProps,mapDispatchToProps)(MatchTheColumnsScreen)